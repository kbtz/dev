...
export class WGL extends Context {
	...
	flush(program, name, value) {
		let location = program.locations[name]
		if (!location) {
			//console.warn('location not found', program.name, name)
			return false
		}
		...
		//console.info('flushed', program.name, name, value)
